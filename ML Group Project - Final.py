...
splot = sns.displot(sat, x="Departure Delay in Minutes", row="satisfaction_v2", binwidth=60, height=3, facet_kws=dict(margin_titles=True))
splot.set(yscale="log")
plt.show()

splot = sns.displot(sat, x="Arrival Delay in Minutes", row="satisfaction_v2", binwidth=60, height=3)
splot.set(yscale="log")
plt.show()

splot = sns.displot(sat, x="Flight Distance", row="satisfaction_v2", binwidth=60, height=3)
plt.show()

ordinal(sat, 'Age', 'satisfaction_v2')
# Age does not seem to correlate with satisfaction at all

splot = sns.displot(sat, x="Age", row="satisfaction_v2", binwidth=5, height=3)
plt.show()

# boxplot
splot = sns.boxplot(x="satisfaction_v2", y="Age", hue="satisfaction_v2", palette=["m", "g"], data=sat)
sns.despine(offset=10, trim=True)
plt.show()

# look at age and gender together
splot = sns.displot(sat, x="Age", row="satisfaction_v2", col = 'Gender', binwidth=5, height=3)
plt.show()

# look at age and class together
splot = sns.displot(sat, x="Age", row="satisfaction_v2", col = 'Class', binwidth=5, height=3)
plt.show()

# look at age and type of travel together
splot = sns.displot(sat, x="Age", row="satisfaction_v2", col = 'Type of Travel', binwidth=5, height=3, facet_kws=dict(margin_titles=True))
plt.show()

# look at age and customer type together
splot = sns.displot(sns.displot(sat, x="Age", row="satisfaction_v2", col = 'Customer Type', binwidth=5, height=3))
plt.show()
...
grid2 = sns.FacetGrid(likert_df, col="question", hue="question", palette="tab20c", col_wrap=4, height=3)
grid2.map(plt.axhline, y=0, ls=":", c=".5")                                         # Draw a horizontal line to show the starting point
grid2.map(plt.plot, "response", "mean", marker="o")                                 # Draw a line plot to show the trajectory of each random walk
grid2.set(xticks=np.arange(5), yticks=[0, 1], xlim=(0.5, 5.5), ylim=(0, 1.5))       # Adjust the tick positions and labels
grid2.fig.tight_layout(w_pad=1)                                                     # Adjust the arrangement of the plots
...
mlp_model.get_params(deep=True)

mlp_model.out_activation_
...
# try heatmap for likerts
# tune random forest
# powerpoint
# do charts and tables and graphs
# anova on population difference

sat.groupby(['Gender']).size().reset_index(name='Counts')

print("ADAM", adam_class)
print("L-BFGS", lbfgs_class)
print("SGD", sgd_class)
sgdstr = sgd_class.split()
column_heads = sgdstr[:4]
dissat = sgdstr[5:9]
sgdsat = sgdstr[10:14]
sgdacc = ["accuracy:"] + sgdstr[14:17]

print(column_heads, dissat, sgdsat, sgdacc)

data = [dissat, sgdsat, sgdacc]

# Create the pandas DataFrame
classrepo = pd.DataFrame(data, columns=column_heads)
classrepo
